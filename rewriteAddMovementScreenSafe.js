...
// Find indices
const importStart = 37;
const importEnd = 49;

const returnStart = lines.findIndex(l => l.startsWith('  return ('));
const returnEnd = lines.findIndex((l, i) => i > returnStart && l.startsWith('  );'));

if (importStart === -1 || importEnd === -1 || returnStart === -1 || returnEnd === -1) {
  console.log('Error finding indices', importStart, importEnd, returnStart, returnEnd);
  process.exit(1);
}

const newImports = `import { CAMouvementTopBar } from '@/components/create-mouvement/CAMouvementTopBar';
...
import { TextInput, ActivityIndicator } from 'react-native';`.split('\n');
...
const stylesIndex = lines.findIndex(l => l.startsWith('const styles = StyleSheet.create({'));
const newStyles = `const styles = StyleSheet.create({
  ...
  btnValidateText: { fontSize: 15, fontWeight: '700', color: CA_THEME.white },`.split('\n');

// Perform replacements from bottom to top to preserve indices
lines.splice(stylesIndex, 1, ...newStyles);
lines.splice(returnStart, returnEnd - returnStart + 1, ...newRender);
lines.splice(importStart, importEnd - importStart + 1, ...newImports);
